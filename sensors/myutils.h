...
inline QString SensorSocketPath(int32_t uid, const QString &userName)
{
    return containerPath(uid, userName) + "/sockets/kmre_sensors";
}
...
inline QString getUnixDomainSocketPath()
{
    ...
    if (!userName.isEmpty()) {
        socketPath = SensorSocketPath(uid, userName);
    }
    else {
