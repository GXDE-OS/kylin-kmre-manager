...
struct SockAddr {
    ...
    int initFromAddrAndPort(const char* addr, int port) {
        in_addr_t a = inet_addr(addr);
        return initFromInet(ntohl(a), port);
    }
...
{
    SockAddr addr;
    int ret = addr.initFromAddrAndPort("0.0.0.0", port);
    //int ret = addr.initFromAddrAndPort("127.0.0.1", port);
    if (ret < 0) {
        return ret;
    }
    return socketBindInternal(&addr, socketType);
...
int socketTcpClient(const char* hostname, int port, int socketType) {
    ...
    SockAddr addr;
    int ret = addr.initFromAddrAndPort(hostname, port);
    if (ret < 0) {
        return ret;
    }
    return socketConnectInternal(&addr, socketType);
